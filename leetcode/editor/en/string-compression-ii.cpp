...
//the run). For example, to compress the string "aabccc" we replace "aa" by "a2" 
//and replace "ccc" by "c3". Thus the compressed string becomes "a2bc3". 
//
...
// 
//
// Related Topics String Dynamic Programming 👍 862 👎 67
...
class Solution {
 public:
  ...
  void runLength(string &s) {
    ...
    size_t cur = 0;
    size_t nextPos = 0;
    while ((nextPos = s.find_first_not_of(s[cur])) != string::npos) {
      string count = to_string(nextPos - cur);
      s.replace(cur + 1, nextPos - cur, count);
      cur = cur + count.size() + 1;
    }
    if (nextPos != s.size() - 1) {
      string count = to_string(s.size() - nextPos);
      s.replace(nextPos, s.size() - nextPos, count);
    }
  }
};
